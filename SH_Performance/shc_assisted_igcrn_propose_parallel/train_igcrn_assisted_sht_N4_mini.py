...
parser.add_argument('--train_wav_scp', type=str,
                    default='/ddnstor/imu_panjiahui/dataset/SHdomainNew/loader_txt_cir_uniform_9/wav_scp_train_23670.txt')
parser.add_argument('--train_mix_dir', type=str,
                    default='/ddnstor/imu_panjiahui/dataset/SHdomainNew/TIMIT/generated_data_cir_uniform_9/train/mix')
parser.add_argument('--train_ref_dir', type=str,
                    default='/ddnstor/imu_panjiahui/dataset/SHdomainNew/TIMIT/generated_data_cir_uniform_9/train/noreverb_ref')
parser.add_argument('--train_mic_dir', type=str,
                    default='/ddnstor/imu_panjiahui/dataset/SHdomainNew/RIR/cir_uniform_9/train_val_rir/MIC')

parser.add_argument('--val_wav_scp', type=str,
                    default='/ddnstor/imu_panjiahui/dataset/SHdomainNew/loader_txt_cir_uniform_9/wav_scp_val_2580.txt')
parser.add_argument('--val_mix_dir', type=str,
                    default='/ddnstor/imu_panjiahui/dataset/SHdomainNew/TIMIT/generated_data_cir_uniform_9/val/mix')
parser.add_argument('--val_ref_dir', type=str,
                    default='/ddnstor/imu_panjiahui/dataset/SHdomainNew/TIMIT/generated_data_cir_uniform_9/val/noreverb_ref')
...
if __name__ == "__main__":
    ...
    if args.resume:
        ...
        modelname = modelpath + 'network_epoch20.pth'
        network = IGCRN(in_ch_sph=sph_channel)
    ...
    for epoch in range(args.num_epoch):
        ...
        for idx, egs in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            stft_input = egs['mix_input'].cuda()
            target = egs['target'].cuda()

            outputs = network(sph_input, stft_input, target.shape[-1])
        ...
        for idx, egs in tqdm(enumerate(val_loader), total=len(val_loader)):
            ...
            with torch.no_grad():
                outputs = network(sph_input, stft_input, target.shape[-1])  # torch.Size([4, 16, 64000])
