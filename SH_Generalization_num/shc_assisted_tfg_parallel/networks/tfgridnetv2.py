...
class TFGridNetV2(AbsSeparator):
    ...
    def __init__(
        ...
        self.conv_shc = nn.Sequential(
            nn.Conv2d(2 * n_shc, emb_dim, ks, padding=padding),
            nn.GroupNorm(1, emb_dim, eps=eps),
        )

        self.conv_stft = nn.Sequential(
            nn.Conv2d(2 * n_stft, emb_dim, ks, padding=padding),
            nn.GroupNorm(1, emb_dim, eps=eps),
        )
        ...
        self.fusion_conv = nn.Conv2d(32 * 2, 32, 1, bias=False)

    def forward(
            self,
            input_shc: torch.Tensor,
            ilens: torch.Tensor,
            input_stft: Optional[Dict] = torch.Tensor,
        ...
        """
        n_samples = input_shc.shape[1]

        shc_mix_std_ = torch.std(input_shc, dim=(1, 2), keepdim=True)  # [B, 1, 1]
        shc_input = input_shc / shc_mix_std_  # RMS normalization
        shc_batch = self.enc(shc_input, ilens)[0]  # [B, T, M, F]
        shc_batch0 = shc_batch.transpose(1, 2)  # [B, M, T, F]
        shc_batch = torch.cat((shc_batch0.real, shc_batch0.imag), dim=1)  # [B, 2*M, T, F]
        # shc_n_batch, _, shc_n_frames, shc_n_freqs = shc_batch.shape

        stft_mix_std_ = torch.std(input_stft, dim=(1, 2), keepdim=True)  # [B, 1, 1]
        stft_input = input_stft / stft_mix_std_  # RMS normalization
        stft_batch = self.enc(stft_input, ilens)[0]  # [B, T, M, F]
        stft_batch0 = stft_batch.transpose(1, 2)  # [B, M, T, F]
        stft_batch = torch.cat((stft_batch0.real, stft_batch0.imag), dim=1)  # [B, 2*M, T, F]
        n_batch, _, n_frames, n_freqs = stft_batch.shape

        shc_batch = self.conv_shc(shc_batch)  # [B, -1, T, F]
        stft_batch = self.conv_stft(stft_batch)

        batch = self.fusion_conv(torch.cat([shc_batch, stft_batch], dim=1))
        batch0 = batch.transpose(1, 2)
        ...
        batch = batch * stft_mix_std_  # reverse the RMS normalization
...
if __name__ == '__main__':
    ...
    input_1 = Variable(torch.FloatTensor(torch.rand(2, 16000, 25)))
    input_3 = Variable(torch.FloatTensor(torch.rand(2, 16000, 1)))
    input_2 = Variable(torch.randint(15999, 16001, (2,)))

    net = TFGridNetV2(input_dim=None, n_srcs=1, n_fft=512, stride=256, n_shc=25, n_stft=1, n_layers=3,
                      lstm_hidden_units=128,
                      attn_approx_qk_dim=256, emb_dim=32)
    macs, params = profile(net, inputs=(input_1, input_2, input_3,))
