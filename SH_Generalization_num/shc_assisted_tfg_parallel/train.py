...
if __name__ == "__main__":
    ...
    if resume:
        ...
        network = TFGridNetV2(input_dim=None, n_srcs=1, n_fft=512, stride=256, n_shc=25, n_stft=1, n_layers=3,
    ...
    for epoch in range(args.num_epoch):
        ...
        for idx, egs in tqdm(enumerate(train_loader), total=len(train_loader)):
            # stft_input = egs['stft_input'][:, 0, :].unsqueeze(1).to(device)
            # shc_input = egs['shc_input'].to(device)
            # target = egs['target'][:, 0, :].to(device)

            # B X C X T
            stft_input = egs['stft_input'][:, 0, :].unsqueeze(1).cuda()
            shc_input = egs['shc_input'].cuda()
            target = egs['target'][:, 0, :].cuda()
            file_name = egs['file_name']

            ilens = shc_input.size(2) * torch.ones(shc_input.size(0), dtype=torch.int64)
            outputs = network(shc_input.transpose(1, 2), ilens, stft_input.transpose(1, 2))
