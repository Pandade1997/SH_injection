...
class NET(nn.Module):
    def __init__(self, channels=20, in_shc=25 * 2):
        super().__init__()
        self.act = nn.ELU()
        self.n_fft = 319
        self.hop_length = 160
        ...
        self.in_ch_lstm = CH_LSTM_F(in_shc, channels, channels)
        self.in_conv = nn.Conv2d(in_channels=in_shc + channels, out_channels=channels, kernel_size=(1, 1))
...
def complexity():
    inputs = torch.randn(1, 25, 16000)
    model = NET()
    output = model(inputs)
    print(output.shape)

    from ptflops import get_model_complexity_info
    mac, param = get_model_complexity_info(model, (25, 16000), as_strings=True, print_per_layer_stat=True, verbose=True)
    print(mac, param)
