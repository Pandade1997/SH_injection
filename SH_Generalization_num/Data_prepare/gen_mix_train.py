...
def mix_noise(clean, noise, snr, channels=channels):
    '''
...
def add_reverb(cln_wav, rir_wav, channels=channels, predelay=50, sample_rate=16000):
    """
    add reverberation
    args:
        cln_wav: L
...
def generate_data(clean_path, strat_time, noise_path, rir_path, snr, scale, segment_length=16000 * 4,
                  channels=channels):
...
def get_clean_chunk(clean_path, clean_chunk_path, sample_rate=16000, chunk=4, num_process=32):
    '''
...
def get_mix_config(clean_chunk_path, noise_path, rir_path, config_path, snr_range=[-10, 10], scale_range=[2, 9]):
    '''
    ...
    for line in clean_lines:
        ...
        snr = np.random.randint(*snr_range)  # snr range is [0, 30)
        scale = np.random.randint(*scale_range) / 10  # scale range is [0.2, 0.9)
        # print(snr)
        # print(scale)
...
def get_data(config_path, save_dir, chunk=4, sample_rate=16000, num_process=32):
    '''
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--clean_wav_list',
        type=str,
        default='/ddnstor/imu_panjiahui/dataset/SHGeneralization/cir_random_num_TVT/multi_channel/Mic8_2s_gpurir/loader_txt/wav_list/train_clean_list.txt',
    ...
    parser.add_argument(
        '--noise_wav_list',
        type=str,
        default='/ddnstor/imu_panjiahui/dataset/SHGeneralization/cir_random_num_TVT/multi_channel/Mic8_2s_gpurir/loader_txt/wav_list/train_noise_list.txt',
    ...
    parser.add_argument(
        '--rir_wav_list',
        type=str,
        default='/ddnstor/imu_panjiahui/dataset/SHGeneralization/cir_random_num_TVT/multi_channel/Mic8_2s_gpurir/loader_txt/rir/rir_train_val.txt',
    ...
    parser.add_argument(
        '--mix_config_path',
        type=str,
        default='/ddnstor/imu_panjiahui/dataset/SHGeneralization/cir_random_num_TVT/multi_channel/Mic8_2s_gpurir/generated_data/train/mix_train.config',
    ...
    parser.add_argument(
        '--save_dir',
        type=str,
        default='/ddnstor/imu_panjiahui/dataset/SHGeneralization/cir_random_num_TVT/multi_channel/Mic8_2s_gpurir/generated_data/train',
    ...
    # 把生成的数据路径写入wav_scp
    file_paths = []
    for root, dirs, files in os.walk(args.save_dir + '/mix'):
        for file in files:
    ...
    txt_path = os.path.join(
        '/ddnstor/imu_panjiahui/dataset/SHGeneralization/cir_random_num_TVT/multi_channel/Mic8_2s_gpurir/loader_txt/wav_scp/wav_scp_train.txt')
