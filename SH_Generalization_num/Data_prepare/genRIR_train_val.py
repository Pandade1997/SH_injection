...
def genRIR(RIR_data_path, MIC_data_path, num_rirs):
    ...
    # 设置房间大小和RT60范围
    room_size = [6, 5, 4]
    t60 = np.array([random.randint(1, 10) for _ in range(num_rirs)]) / 10  # 训练集的混响：0.2-1

    # 生成麦克风位置
    mic_array_radius = 0.35  # 10cm
    num_mics = 8
    ...
    for i in range(num_rirs):
        ...
        # src_pos = np.stack((src_pos_1, src_pos_2), axis=0)
        # np.array([list(src_pos_1), list(src_pos_1)])
...
if __name__ == '__main__':
    # 定义目录名
    out_path = '/ddnstor/imu_panjiahui/dataset/SHGeneralization/cir_random_num_TVT/multi_channel/Mic8_2s_gpurir/RIR/cir_uniform_8/train_val_rir/'
    txt_path = '/ddnstor/imu_panjiahui/dataset/SHGeneralization/cir_random_num_TVT/multi_channel/Mic8_2s_gpurir/loader_txt/rir/'
    ...
    # 将文件路径写入txt文件
    RIR_txt_path = os.path.join(txt_path, "rir_train_val.txt")
    with open(RIR_txt_path, "w") as f:
        for file_path in RIR_file_paths:
            f.write(file_path + "\n")

    MIC_txt_path = os.path.join(txt_path, "mic_train_val.txt")
    with open(MIC_txt_path, "w") as f:
        for file_path in MIC_file_paths:
            f.write(file_path + "\n")

    # 创建 train val test 的clean和noise的list
    # ls -R /ddnstor/imu_panjiahui/dataset/SHGeneralization/MS-SNSD-master-TVT/clean_train/*.wav > /ddnstor/imu_panjiahui/dataset/SHGeneralization/cir_random_num_TVT/multi_channel/Mic8_2s_gpurir/loader_txt/wav_list/train_clean_list.txt
    # ls -R /ddnstor/imu_panjiahui/dataset/SHGeneralization/MS-SNSD-master-TVT/noise_train/*.wav > /ddnstor/imu_panjiahui/dataset/SHGeneralization/cir_random_num_TVT/multi_channel/Mic8_2s_gpurir/loader_txt/wav_list/train_noise_list.txt
    #
    # ls -R /ddnstor/imu_panjiahui/dataset/SHGeneralization/MS-SNSD-master-TVT/clean_val/*.wav > /ddnstor/imu_panjiahui/dataset/SHGeneralization/cir_random_num_TVT/multi_channel/Mic8_2s_gpurir/loader_txt/wav_list/val_clean_list.txt
    # ls -R /ddnstor/imu_panjiahui/dataset/SHGeneralization/MS-SNSD-master-TVT/noise_val/*.wav > /ddnstor/imu_panjiahui/dataset/SHGeneralization/cir_random_num_TVT/multi_channel/Mic8_2s_gpurir/loader_txt/wav_list/val_noise_list.txt
    #
    #
    # ls -R /ddnstor/imu_panjiahui/dataset/SHGeneralization/MS-SNSD-master-TVT/clean_test/*.wav > /ddnstor/imu_panjiahui/dataset/SHGeneralization/cir_random_num_TVT/multi_channel/Mic8_2s_gpurir/loader_txt/wav_list/test_clean_list.txt
    # ls -R /ddnstor/imu_panjiahui/dataset/SHGeneralization/MS-SNSD-master-TVT/noise_test/*.wav > /ddnstor/imu_panjiahui/dataset/SHGeneralization/cir_random_num_TVT/multi_channel/Mic8_2s_gpurir/loader_txt/wav_list/test_noise_list.txt
