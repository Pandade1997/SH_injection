...
def genRIR(RIR_data_path, MIC_data_path, num_rirs, num_mics):
    fs = 16000
    # 设置房间大小和RT60范围
    room_size = [6, 5, 4]
    ...
    for i in range(num_rirs):
        ...
        # 保存RIR和麦克风位置信息
        np.save(MIC_data_path + '/mic_' + str(i) + '_' + str(t60[i]) + '.npy', mic_pos)
...
if __name__ == '__main__':
    ...
    num_mics_list = [4, 8, 12, 16]
    for num_mics in num_mics_list:
        # 定义目录名
        out_path = '/ddnstor/imu_panjiahui/dataset/SHGeneralization/cir_random_num_TVT/multi_channel/Mic8_2s_gpurir/RIR/cir_uniform_8/test_rir/mic_' + str(
            num_mics)
        ...
        # 1、生成房间冲激响应，在V100上运行
        genRIR(RIR_data_path, MIC_data_path, num_rirs, num_mics)
        ...
        # 将文件路径写入txt文件
        RIR_txt_path = os.path.join(txt_path, "rir_test_mic_" + str(num_mics) + ".txt")
        with open(RIR_txt_path, "w") as f:
            for file_path in RIR_file_paths:
                f.write(file_path + "\n")

        MIC_txt_path = os.path.join(txt_path, "mic_test_mic_" + str(num_mics) + ".txt")
