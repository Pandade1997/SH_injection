...
if __name__ == '__main__':
    # 定义目录名
    out_path = '/ddnstor/imu_panjiahui/dataset/SHdomainShcAssisted/RIR/cir_uniform_9/val_rir/'
    ...
    # 将文件路径写入txt文件
    RIR_txt_path = os.path.join(txt_path, "val_rir.txt")
    with open(RIR_txt_path, "w") as f:
        for file_path in RIR_file_paths:
            f.write(file_path + "\n")

    MIC_txt_path = os.path.join(txt_path, "val_mic.txt")
    with open(MIC_txt_path, "w") as f:
        for file_path in MIC_file_paths:
            f.write(file_path + "\n")

    # 创建 train val test 的clean和noise的list
    # ls -R /ddnstor/imu_panjiahui/dataset/SHdomainNew/TIMIT/Data/train_clean/*.WAV > /ddnstor/imu_panjiahui/dataset/SHdomainShcAssisted/data_txt/train_clean_list.txt
    # ls -R /ddnstor/imu_panjiahui/dataset/SHdomainNew/TIMIT/Data/val_clean/*.WAV > /ddnstor/imu_panjiahui/dataset/SHdomainShcAssisted/data_txt/val_clean_list.txt
    # ls -R /ddnstor/imu_panjiahui/dataset/SHdomainNew/TIMIT/Data/test_clean/*.WAV > /ddnstor/imu_panjiahui/dataset/SHdomainShcAssisted/data_txt/test_clean_list.txt
    #
    # find /ddnstor/imu_panjiahui/dataset/dns-challenge-2/datasets/noise/ -name "*.wav" | xargs -i ls -R {} > /ddnstor/imu_panjiahui/dataset/SHdomainShcAssisted/data_txt/train_val_noise_list.txt
    # ls -R /ddnstor/imu_panjiahui/dataset/DIVERSE_multi_dataset/NOISE92X_CHIME3/*.wav > /ddnstor/imu_panjiahui/dataset/SHdomainShcAssisted/data_txt/test_noise_list.txt
